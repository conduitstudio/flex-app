...
(function($) {
	...
	$.fn.Slider = function() {
        ...
        $reveals = $stage.find('.stage-reveal:not(:first-child)'),
        $products = $page.find('.products .product:not(:first-child)'); // <= first child is always full width
      ...
      const slider = new Dragdealer( $self[0], {
        ...
        animationCallback: function(x, y) {
          ...
          $active_reveal.width( reveal_width_perc * 100 + '%' );
					...
					// Set opacity of products too
					$products.eq( active_reveal_index ).css('opacity', reveal_width_perc );
          ...
          // if( x > divided_by ) $reveals.eq(0).width( '100%' ).removeClass('in-process');
					//
          // if( x <= divided_by ) $reveals.eq(1).width( '0%' ).removeClass('in-process');
          // if( x > (divided_by*2) ) $reveals.eq(1).width( '100%' ).removeClass('in-process');
					//
          // if( x <= (divided_by*2) ) $reveals.eq(2).width( '0%' ).removeClass('in-process');
        ...
        dragStopCallback: function(x,y) {
          ...
          this.setValue( (Math.round(x * $reveals.length) / $reveals.length).toFixed(3), y, false );
					...
					console.log('dragStopCallback', x)
        }
      });
			...
			// Slider-jumps
			$self.siblings('.slider-jumps').find('a').on('click.flex', function(e) {
				e.preventDefault();
