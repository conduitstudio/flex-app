...
Flex.on_openmodal = function(e) {
  ...
  $modal.addClass('in');
	...
	if( $modal.find('.video').length > 0 && $modal.find('.video').hasClass('played') ) {
    ...
    const $target = $(e.target);
		...
		// console.log({
		// 	1: ! $target.parent().hasClass('holder'),
		//       2: $target.closest('.container').length === 0,
		//       3: $target.parent('[data-openmodal]').length === 0,
		// });
